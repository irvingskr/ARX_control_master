...
arx_arm::arx_arm(int CONTROL_MODE)
{
    ...
    if (play.modifyLinkMass(model_path, model_path, 0.640)) { //默认单位kg 夹爪质量 0.381 0.581
...
void arx_arm::update_real(command cmd)
{
    ...
    ROS_INFO("solve pos x>%f y%f z%f r%f p%f y%f",solve.solve_pos[0],solve.solve_pos[1],solve.solve_pos[2],solve.solve_pos[3],solve.solve_pos[4],solve.solve_pos[5],solve.solve_pos[6]);
    if(is_starting)
    {
...
float arx_arm::Data_process(float& Data)
{
    ...
    // //低通滤波,对手部的抖动进行消除(消除抖动的效果比较好，但是存在一定的延时)
    ...
    // 均值滤波，对手部的抖动进行消除(基本感觉不到延时，略微有些抖动)
    Data_filter[0] = Data_filter[1];
    Data_filter[1] = Data_filter[2];
    Data_filter[2] = Data_filter[3];
    Data_filter[3] = Data_filter[4];
    Data_filter[4] = Data_filter[5];
    Data_filter[5] = Data_filter[6];
    Data_filter[6] = Data_filter[7];
    Data_filter[7] = Data_filter[8];
    Data_filter[8] = Data_filter[9];
    Data_filter[9] = Data;
    float sum = 0;
	for ( auto x : Data_filter )
		sum += x;
    return sum*0.1;
}
void arx_arm::motor_control()
{
    ...
    if(current_normal)
    {
            if (is_teach_mode) {
                // 拖动模式
                if(control_mode == 0 || control_mode==2){
                    CAN_Handlej.Send_Moto_1 (1, 0, 0, 0, 0,  solve.joint_torque[0]);}else if(control_mode == 1 || control_mode==3){
                    CAN_Handlej.Send_Moto_2 (1, 0, 0, 0, 0,  solve.joint_torque[0]);
                }usleep(200);

                CAN_Handlej.Send_Moto_1(2, 0, 0, 0, 0,  solve.joint_torque[1]);usleep(200);
                CAN_Handlej.Send_Moto_1(4, 0, 0, 0, 0,  solve.joint_torque[2]);usleep(200);
                ...
                CAN_Handlej.Send_Moto_2(8, 0, 0.3, 0, 0,  ros_control_cur_t[6]*0.6);  usleep(200);
                       ...
                       if(control_mode == 0 || control_mode==2){
                            CAN_Handlej.Send_Moto_1 (1, 150, 12, target_pos[0], 0, solve.joint_torque[0]);
                        }else if(control_mode == 1 || control_mode==3){
                            CAN_Handlej.Send_Moto_2(1, 39, 0.8, target_pos[0], 0, solve.joint_torque[0]);
                        }
                        CAN_Handlej.Send_Moto_1(2, 150, 12, target_pos[1], 0, solve.joint_torque[1]); usleep(200);
                        CAN_Handlej.Send_Moto_1(4, 150, 12, target_pos[2], 0, solve.joint_torque[2]); usleep(200);
    ...
    {               
                    CAN_Handlej.Send_Moto_1(1, 0, 12, 0, 0, 0);usleep(200);
                    CAN_Handlej.Send_Moto_1(2, 0, 12, 0, 0, 0);usleep(200);
                    CAN_Handlej.Send_Moto_1(4, 0, 12, 0, 0, 0);usleep(200);
...
void arx_arm::init_step()
{
   ...
   if(temp_init>2)
   {
            ...
            if(control_mode == 0 || control_mode == 2){
                CAN_Handlej.Send_Moto_1 (1, init_kp, init_kd, target_pos[0], 0,  0); }else if(control_mode == 1 || control_mode == 3){

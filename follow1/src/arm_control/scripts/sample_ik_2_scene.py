...
step = 0
Max_step = 500
directory_path = f'/media/dc/ESD-USB/ARX_REAL_TEST'# f'/media/dc/HP2024/data/SCIL/Task4_long_horizon'
extension = '.hdf5' 
...
# def callback(JointCTR2,JointInfo2,f2p,image_mid):
    ...
    #     invalid = np.abs(qpos-data_dict["/observations/qpos"][-1]) <= 0.03
    # invalid = len(data_dict["/observations/qpos"])<0 or np.abs(np.mean(qpos-data_dict["/observations/qpos"][-1])) <= 0.03
...
if __name__ =="__main__":
    ...
    rospy.init_node("sample_ik_2_scene")
    ...
    ats = ApproximateTimeSynchronizer([master1,follow1,follow1_pos,image_mid, image_left],slop=0.3,queue_size=2)

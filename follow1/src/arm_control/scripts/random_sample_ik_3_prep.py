...
step = 0
Max_step = 1000
directory_path = f'/media/dc/XiaoXiong/arx_rand_diverse'# f'/media/dc/HP2024/data/SCIL/Task4_long_horizon'
extension = '.hdf5' 
...
def callback(JointInfo2,f2p,image_mid,image_right,image_left):
# def callback(JointCTR2,JointInfo2,f2p,image_mid):
    ...
    if len(data_dict["/observations/qpos"])>0: 
        dist_threshold = 0.1
        dist_from_last_qpos = np.linalg.norm(qpos-data_dict["/observations/qpos"][-1])
        # print(dist_from_last_qpos)
        if dist_from_last_qpos <= dist_threshold:
            return
    ...
    canvas = np.zeros((480, 3*640, 3), dtype=np.uint8)
    ...
    if step >= Max_step and save:
        ...
        with h5py.File(dataset_path,'w',rdcc_nbytes=1024 ** 2 * 10) as root:
            ...
            left_images = root['/observations/images/left'][...]
            images = np.concatenate([mid_images,right_images,left_images],axis=2)
...
if __name__ =="__main__":
    ...
    follow1 = Subscriber("joint_information2",JointInformation)
    ...
    ats = ApproximateTimeSynchronizer([follow1,follow1_pos,image_mid, image_right, image_left],slop=0.3,queue_size=2)
